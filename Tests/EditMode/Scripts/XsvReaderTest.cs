    ...
    public class XsvReaderTest
    {
        ...
        public void main()
        {
            ...
            Debug.Log(string.Format("TextAsset:\n{0}", rowsAsset.text.Replace("\n", "\\n")));
            {
                ...
                var list = XsvReader.GetList(XsvParser.Delimiter.Comma, rowsAsset);
                Debug.Log(string.Format("Case1 GetList List/List; List[0][0]:{0}, List[1][0]:{1}", list[0][0], list[1][0]));
            }
            {
                // case 2 Return List/List NoneHeader
                var list = XsvReader.GetList(XsvParser.Delimiter.Comma, rowsAsset, false);
                Debug.Log(string.Format("Case2 GetList List/List NoneHeader; List[0][0]:{0}, List[1][0]:{1}", list[0][0], list[1][0]));
            }
            {
                // case 3 Return List/Dictionary
                var list = XsvReader.GetListWithHeader(XsvParser.Delimiter.Comma, rowsAsset);
            ...
            }
            {
                ...
                var list = XsvReader.GetList<SampleColumns>(XsvParser.Delimiter.Comma, rowsAsset);
            ...
            }
            {
                ...
                var list = XsvReader.GetList<SampleColumns>(XsvParser.Delimiter.Comma, rowsAsset, false);
                ...
                "Current Example Case; m_Delimiter:Comma, m_XsvAsset = rowsAsset, m_HeaderEnable = true");
            {
                ...
                var list = m_XsvData.GetList<SampleColumns>();
